...
from apps.core.cache_utils import ExpenseSystemCache, cache_result
import logging
...
class OptimizedExpenseClaimListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not user.has_perm('expense_claims.can_view_all_claims'):
...
class OptimizedExpenseClaimDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        # Check permissions
        user = self.request.user
        if not user.has_perm('expense_claims.can_view_all_claims') and obj.claimant != user:
...
class DashboardView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        role = 'admin' if user.is_staff else ('manager' if user.has_perm('expense_claims.can_approve_claims') else 'employee')
...
def claim_edit_view(request, pk):
    ...
    # Check permissions
    if claim.claimant != request.user and not request.user.has_perm('expense_claims.can_edit_all_claims'):
        messages.error(request, 'You do not have permission to edit this claim.')
        return redirect('expense_claims:claim_detail', pk=pk)
...
def claim_delete_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('expense_claims:claim_list')
...
def pending_approvals_view(request):
    ...
    if not request.user.has_perm('expense_claims.can_approve_claims'):
