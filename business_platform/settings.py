"""
Django settings for Integrated Business Platform.

CG Global Entertainment Ltd - Integrated Business Management Platform
Includes: Expense Claims, Documents, Reports, and User Management
"""
...
ALLOWED_HOSTS = config('ALLOWED_HOSTS', default='localhost,127.0.0.1,192.168.0.104', cast=lambda v: [s.strip() for s in v.split(',')])
...
LOCAL_APPS = [
    ...
    "apps.reports",
    "apps.core",
    # NOTE: leave_management and stripe_management moved to separate apps
    # - Leave system: company-leave-system (port 8002)
    # - Stripe dashboard: stripe-dashboard (port 8081)
]
...
# Custom User Model (Platform-wide)
...
CACHES = {
    'default': {
        ...
        'LOCATION': 'business_platform_cache',
...
CSRF_TRUSTED_ORIGINS = config('CSRF_TRUSTED_ORIGINS', default='http://localhost:8081,http://127.0.0.1:8081,http://192.168.0.104:8084,http://localhost:8084', cast=lambda v: [s.strip() for s in v.split(',')])
...
LOGGING = {
    ...
    'handlers': {
        'file': {
            ...
            'filename': BASE_DIR / 'logs' / 'business_platform.log',
            'formatter': 'verbose',
        },
        'console': {
            'level': 'INFO',
